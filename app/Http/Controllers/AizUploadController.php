...

{
    ...
    public function upload(Request $request){
        ...
        $skuRes = isset($request->sku) ? $request->sku : '';
...

    {
        ...
        $skuPro =  isset($request->sku) ? $request->sku : '';

        if($skuPro !=''){

            $uploads->where('prosku', '!=','');
...

    {
        ...
        $all_uploads->join('products','products.stock_id','=','uploads.prosku')
        ->select('uploads.*','products.stock_id');
        // ->where('products.stock_id','=','uploads.prosku');
        if ($request->search != null) {
        ...
        $all_uploads = $all_uploads->paginate(60)->appends(request()->query());
        return view('backend.uploaded_files.product_upload', compact('all_uploads', 'search', 'sort_by'));
    }
