...
class HomeController extends Controller
{
    ...
    * Show the customer/seller dashboard.
    ...
    {
        $detailedProduct  = Product::with('reviews', 'brand', 'stocks', 'user', 'user.shop','category','productcondition','productType')->where('slug', $slug)->where('approved', 1)->first();
        ...
        if($detailedProduct != null && $detailedProduct->published){
            ...
            if($detailedProduct->digital == 1){
               return view('frontend.digital_product_details', compact('detailedProduct'));
                ...
                $modelNumber=$detailedProduct->model;
                $stock_id = $detailedProduct->stock_id;
                $listning_type= Producttype::where('id','=',$detailedProduct->product_type_id)->first()->listing_type;
                $prodQtyArr = Product::select(DB::raw('SUM(product_stocks.qty) as prostock'))
                ->join('product_stocks','products.id','=','product_stocks.product_id')
               ->where('products.published', '=', '1')
                ->where('products.model', '=', $modelNumber)
                 ->groupBy('products.model')
                 ->first();
                $prostock = isset($prodQtyArr->prostock) ? $prodQtyArr->prostock : '';
                $linkpage = 'https://gciwatch/'.$listning_type.'/'.$modelNumber.'/'.$stock_id;
                return view('frontend.product_details', compact('detailedProduct','linkpage','products', 'prostock'));
            }
        }
        abort(404);
    }
