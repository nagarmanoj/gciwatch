...
<div class="aiz-titlebar text-left mt-2 mb-3">
	<div class="align-items-center">
		<!-- <h1 class="h3">{{translate('All Job Orders')}}</h1> -->
	</div>
</div>
...
<div class="row">
    <div class="col-md-12">
        <div class="dropdown mb-2 mb-md-0 trans_mi">
            ...
            <button class="btn border dropdown-toggle" type="button" data-toggle="dropdown"><i class="las la-bars fs-20"></i></button>
            <div class="dropdown-menu dropdown-menu-right">
            @if(Auth::user()->user_type == 'admin' || in_array('36', json_decode(Auth::user()->staff->role->inner_permissions)))
                <div class="delete_section"  height="30px;">
        ...
        <div class="card" style="margin-top:20px;">
            ...
            <div class="mi_custome_table">
                <form method="get" id="pagination_form" action="{{route('job_orders.index')}}">
                ...
                <div class="card-body">
                    <form class="" id="sort_products" action="" method="GET">
                            ...
                            <tbody>
                                @foreach($jobOrderData  as $key => $jobData)
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            <a style="text-decoration:none;color:#000;" href="{{ route('job_orders.edit',['id'=>$jobData->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}"  title="{{ translate('Edit') }}">
                                                <img src="{{uploaded_asset($jobData->image_upon_receipt_job)}}" alet="pro_img">
                                            </a>
                                        </td>
                                        <td>
                                            <a style="text-decoration:none;color:#000;" href="{{ route('job_orders.edit',['id'=>$jobData->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}"  title="{{ translate('Edit') }}">
                                                {{ $jobData->job_order_number}}
                                            </a>
                                        </td>
                                        ...
                                        <td>
                                                <a style="text-decoration:none;color:#000;" href="{{ route('job_orders.edit',['id'=>$jobData->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}"  title="{{ translate('Edit') }}">
                                                {{ date("m/d/20y", strtotime($jobData->estimated_date_return))}}
                                            </a>
                                        </td>
                                        <td>
                                            <a style="text-decoration:none;color:#000;" href="{{ route('job_orders.edit',['id'=>$jobData->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}"  title="{{ translate('Edit') }}">
                                                {{ date("m/d/20y", strtotime($jobData->estimated_date))}}
                                            </a>
                                        </td>
                                        <td>
                                            <a style="text-decoration:none;color:#000;" href="{{ route('job_orders.edit',['id'=>$jobData->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}"  title="{{ translate('Edit') }}">
                                        ...
                                        <td class="text-right" style="height:60px;">
                                            <a class="btn btn-soft-success btn-icon btn-circle btn-sm"  href="{{ route('job_orders.activity',['id'=>$jobData->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" target="_blank" title="{{ translate('Activity') }}">
                                                    <i class="las la-history"></i>
                                            </a>
                                            @if(Auth::user()->user_type == 'admin' || in_array('33', json_decode(Auth::user()->staff->role->inner_permissions)))
                                            <a class="btn btn-soft-success btn-icon btn-circle btn-sm"  href="{{ route('job_orders.view',['id'=>$jobData->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" target="_blank" title="{{ translate('View') }}">
                                                <i class="las la-eye"></i>
                                            </a>
                                            @endif
                                            @if(Auth::user()->user_type == 'admin' || in_array('35', json_decode(Auth::user()->staff->role->inner_permissions)))
                                            <a href="{{ route('job_orders.edit',['id'=>$jobData->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" class="btn btn-soft-success btn-icon btn-circle btn-sm" title="{{ translate('Edit') }}">
                                                <i class="las la-edit"></i>
                                            </a>
                                            @endif
                                            @if(Auth::user()->user_type == 'admin' || in_array('36', json_decode(Auth::user()->staff->role->inner_permissions)))
                                            <a href="{{ route('job_orders.destroy',['id'=>$jobData->id, 'lang'=>env('DEFAULT_LANGUAGE')] )}}" class="btn btn-soft-danger btn-icon btn-circle btn-sm" data-href="#" title="{{ translate('Delete') }}">
                        ...
                        <!-- <p>
                         Showing {{ $jobOrderData->firstItem() }} to {{ $jobOrderData->lastItem() }} of total {{$jobOrderData->total()}} entries
                         </p> -->
                        <div class="aiz-pagination">
                            @if($pagination_qty != "all")
                            {{ $jobOrderData->appends(request()->input())->links() }}
                            @endif
                        </div>
												<!-- <p>
						              Showing {{ $jobOrderData->firstItem() }} to {{ $jobOrderData->lastItem() }} of total {{$jobOrderData->total()}} entries
