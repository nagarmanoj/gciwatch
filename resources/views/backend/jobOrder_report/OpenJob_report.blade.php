...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
        <div class="col-auto">
            <h1 class="h3">{{translate('Open Job Report
')}}</h1>
        </div>
    </div>
</div>
...
<div class="list_bulk_btn">
    ...
    <div class="dropdown-menu dropdown-menu-right">
        <a href="{{Route('openJob_report_export.index')}}" id="xls" class="tip" title="" style="text-decoration:none;" data-original-title="Download as XLS">
            Excel
        </a>
    </div>
</div>
...
<!-- <div class="box-header">
    <h2 class="blue"><i class="fa-fw fa fa-barcode"></i>Open Job Report
    </h2>
    <div class="box-icon">
        <ul class="btn-tasks">
            <li class="dropdown">
                <a href="{{Route('openJob_report_export.index')}}" id="xls" class="tip" title="" data-original-title="Download as XLS">
                   Excel
                </a>
            </li>
        </ul>
...
<div class="card">
    <!-- <form class="" id="sort_products" action="" method="GET"> -->
        ...
        <table class="table aiz-table mb-0 footable footable-1 breakpoint breakpoint-lg list_agent_table" style="">
            ...
            <tbody> 
                @php $count =0; @endphp
                @foreach($OpenJob_report as $key => $row)
                    <?php $count++ ; ?>
                    <tr> 
                         <td>{{ ($OpenJob_report->currentpage()-1) * $OpenJob_report->perpage() + $key + 1 }}</td>
        ...
        <div class="aiz-pagination">
              ...
              <p>
                Showing {{ $OpenJob_report->firstItem() }} to {{ $OpenJob_report->lastItem() }} of  {{$OpenJob_report->total()}} entries
              </p>
                {{ $OpenJob_report->appends(request()->input())->links() }}
                @else
                <p>
                  Showing {{$OpenJob_report->count()}} of  {{$OpenJob_report->count()}} entries
                </p>
              @endif
        </div>
        </div>
