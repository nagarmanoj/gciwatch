...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
        <div class="col-auto">
            <h1 class="h3">{{translate('On Hold Job Report
')}}</h1>
        </div>
    </div>
</div>
...
<div class="dropdown mb-2 mb-md-0 right_drop_mune_sec trans_mi">
    ...
    <div class="dropdown-menu dropdown-menu-right">
        <a href="{{Route('onHoldJob_report_export.index')}}" id="xls" class="tip" title="" style="text-decoration:none;" data-original-title="Download as XLS">
            Excel
        </a>
    </div>
</div>
...
<!-- <div class="box-header">
    <h2 class="blue"><i class="fa-fw fa fa-barcode"></i>On Hold Job Report
    </h2>
    <div class="box-icon">
        <ul class="btn-tasks">
            <li class="dropdown">
                <a href="{{Route('onHoldJob_report_export.index')}}" id="xls" class="tip" title="" data-original-title="Download as XLS">
                   Excel
                </a>
            </li>
        </ul>
...
<div class="card">
    <!-- <form class="" id="sort_products" action="" method="GET"> -->
        ...
        <table class="table aiz-table mb-0 footable footable-1 breakpoint breakpoint-lg list_agent_table" style="">
            ...
            <tbody> 
                @php $count =0; @endphp
                @foreach($OnHoldJob_report as $key => $row)
                    <?php $count++ ; ?>
                    <tr> 
                    <td>{{ ($OnHoldJob_report->currentpage()-1) * $OnHoldJob_report->perpage() + $key + 1 }}</td>
        ...
        <div class="aiz-pagination">
              ...
              <p>
                Showing {{ $OnHoldJob_report->firstItem() }} to {{ $OnHoldJob_report->lastItem() }} of  {{$OnHoldJob_report->total()}} entries
              </p>
                {{ $OnHoldJob_report->appends(request()->input())->links() }}
                @else
                <p>
                  Showing {{$OnHoldJob_report->count()}} of  {{$OnHoldJob_report->count()}} entries
                </p>
              @endif
        </div>
        </div>
