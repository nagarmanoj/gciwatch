...
<div class="aiz-titlebar text-left mt-2 mb-3">
    <div class="row align-items-center">
        <div class="col-auto">
            <h1 class="h3">{{translate('Pending Job Report
')}}</h1>
        </div>
    </div>
</div>
...
<div class="dropdown mb-2 mb-md-0 right_drop_mune_sec trans_mi">
    ...
    <div class="dropdown-menu dropdown-menu-right">
        <a href="{{Route('pendingJob_report_export.index')}}" id="xls" class="tip" title="" style="text-decoration:none;" data-original-title="Download as XLS">
            Excel
        </a>
    </div>
</div>
...
<!-- <div class="box-header">
    <h2 class="blue"><i class="fa-fw fa fa-barcode"></i>Pending Job Report
    </h2>
    <div class="box-icon">
        <ul class="btn-tasks">
            <li class="dropdown">
                <a href="{{Route('pendingJob_report_export.index')}}" id="xls" class="tip" title="" data-original-title="Download as XLS">
                   Excel
                </a>
            </li>
        </ul>
...
<div class="card">
    <!-- <form class="" id="sort_products" action="" method="GET"> -->
        ...
        <table class="table aiz-table mb-0 footable footable-1 breakpoint breakpoint-lg list_agent_table" style="">
            ...
            <tbody> 
                @php $count =0; @endphp
                @foreach($pending_job_report as $key => $row)
                    <?php $count++ ; ?>
                    <tr> 
                 <td>{{ ($pending_job_report->currentpage()-1) * $pending_job_report->perpage() + $key + 1 }}</td>
                       <!--  <td> {{$count}} </td> -->
        ...
        <div class="aiz-pagination">
              ...
              <p>
                Showing {{ $pending_job_report->firstItem() }} to {{ $pending_job_report->lastItem() }} of  {{$pending_job_report->total()}} entries
              </p>
                {{ $pending_job_report->appends(request()->input())->links() }}
                @else
                <p>
                  Showing {{$pending_job_report->count()}} of  {{$pending_job_report->count()}} entries
                </p>
              @endif
        </div>
    </div>
