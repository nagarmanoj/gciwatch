		...
		<div class="card">
		    <div class="card-header row gutters-5">
				<div class="col text-center text-md-left">
					<h5 class="mb-md-0 h6">{{ translate('All Partners') }}</h5>
				</div>
				<!-- <div class="col-md-4">
				...
				<div class="btn-group mr-2" role="group" aria-label="Third group">
					<!-- <a class="btn btn-soft-primary" href="{{route('partners.create')}}" title="{{ translate('add') }}">
						Add Partner	<i class="lar la-plus-square"></i>
					</a> -->
				</div>
		    </div>
		    <div class="card-body">
		        <table class="table mb-0" id="site_partner">
		            <thead>
		                <tr>
		                    <th>#</th>
		                    <th>{{translate('Partners Name')}}</th>
...
<script type="text/javascript">
 $(document).ready(function() {
	 if($('#site_partner').length > 0){
			$('#site_partner').DataTable({
             processing: true,
             serverSide: true,
             ajax: "{{route('partners.getAllpartners')}}",
             columns: [
                 { data: 'id' },
                 { data: 'option_value' },
                 { data: 'description' },
