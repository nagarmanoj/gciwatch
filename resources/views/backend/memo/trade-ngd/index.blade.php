...
<div class="aiz-titlebar text-left mt-2 mb-3">
	...
	<div class="align-items-center">
			...
			<!-- <h1 class="h3">{{translate('TRADE/NGD')}}</h1> -->
...
<div class="dropdown mb-2 mb-md-0 hideonprint trans_mi">
	...
	<div class="dropdown-menu dropdown-menu-right">
		<form class="" action="{{route('memo_tradeNGD_export.index')}}" method="post">
			@csrf
		...
		<div class="card">
		    ...
		    <div class="card-header row gutters-5">
				...
				<div class="col text-center text-md-left">
					...
					<h5 class="mb-md-0 h6">{{ translate('TRADE/NGD') }}</h5>
		    ...
		    <div class="mi_custome_table">
                <form method="get" id="pagination_form" action="{{route('memotrade_ngd.index')}}">
		    ...
		    <div class="card-body">
		        ...
		        <table class="table aiz-table mb-0" id="trade_ngd_data_t">
		            ...
		            <tbody>
		                ...
		                @foreach($memotrade_ngdData  as $key => $trade_ngd)

		                    <tr>

	                          <td style="text-align:center;"><input type="checkbox" class="pro_checkbox" data-id="{{$trade_ngd->memodetailsid}}" name="all_pro[]" value="{{$trade_ngd->memodetailsid}}"></td>

		                        <td>TNGD101{{ $trade_ngd->memoid}}</td>

		                        @if($trade_ngd->customer_group='reseller')
		                        <td>{{ $trade_ngd->company}}</td>
								@else
								<td>{{ $trade_ngd->customer_name}}</td>
								@endif

		                        <td>{{ $trade_ngd->stock_id}}</td>

		                        <td>{{ $trade_ngd->model}}</td>

		                        <td>{{ $trade_ngd->sku}}</td>

		                        <td>{{ $trade_ngd->row_total}}</td>

		                        <td>{{date('m/d/20y',strtotime($trade_ngd->status_updated_date))}}</td>

		                        <td class="text-right">

		                            <!-- <a class="btn btn-soft-primary btn-icon btn-circle btn-sm" href="{{route('memo.viewinvoice',['id'=>$trade_ngd->memoid,'status'=>$trade_ngd->item_status])}}" title="{{ translate('Edit') }}">

		                                <i class="las la-edit"></i>

		                            </a> -->

									<a href="{{route('memo.generatepdf',['id'=>$trade_ngd->memoid,'status'=>$trade_ngd->item_status])}}" class="btn btn-soft-success btn-icon btn-circle btn-sm" title="{{ translate('View') }}">
